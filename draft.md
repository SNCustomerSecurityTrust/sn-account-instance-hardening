# Best Ways to Query ServiceNow for Least Privilege & Security Configurations
...
These scripts are designed to be run in the **Scripts - Background** console of a ServiceNow instance. They assess common security configuration gaps across identity, access control, privileged activity, and platform hygiene.
...
gr.addQuery('state', 'active'); // Role assignment is still active even though the user is not
gr.query();
...
## 3a. Recent Impersonation Activity
...
gr.addQuery('sys_created_on', '>', gs.daysAgo(30)); // Configurable lookback window
...
while (gr.next()) {
    ...
    // Extract the impersonated user from parentheses in the message
    ...
    // Extract who performed the impersonation (only present in start events)
    var impersonatedBy = '';
...
var noMFAUsers = [];
while (gr.next()) {
    // Check if user has admin or security_admin roles
    if (gr.hasRole('admin') || gr.hasRole('security_admin')) {
        // Check for active MFA device enrollment
...
## 11b. Adaptive Authentication Verification
...
(function verifyAdaptiveAuth() {
    ...
    // 3. Query active policy contexts and count their active policies
